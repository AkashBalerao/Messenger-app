...
export async function DELETE(
  ...
  try {
    ...
    if (!isParticipant) {
      // If the current user is not a participant, return 400 Bad Request
      return new NextResponse('User is not a participant in this event', { status: 400 });
    }

    // Update the event to remove the current user's ID from participantIds
    const updatedEvent = await prisma.event.update({
      where: { id },
      data: {
        participants: {
    ...
    return new NextResponse('User removed from event participants', { status: 200 });

  } catch (error) {
    console.error(error, 'Error removing user from event participants');
    // If an error occurs, return a 500 Internal Server Error response
    return new NextResponse('Error removing user from event participants', { status: 500 });
  }
}
